...
const quotes = [
  ...
  "Até meu mailing ruim começou a converter.",
];
    ...
    <div className="max-w-2xl mx-auto text-center">
      ...
      <FadeIn delay={0.15}>
        <div className="space-y-4 mb-8">
          {quotes.map((q, i) => (
            <div
              key={i}
              className="bg-card border border-border rounded-xl px-6 py-4 text-foreground text-sm md:text-base font-medium"
            >
              "{q}"
            </div>
          ))}
      ...
      <FadeIn delay={0.25}>
        <p className="text-muted-foreground/60 text-xs">
          Isso é o que acontece quando você opera no novo modelo.
        </p>
      </FadeIn>
    </div>
  </section>
